...
class StlLoader:
    ...
    def get_triangles_generator(self):
        if self.model:
...
def model_dict_load(model_dict):
	stl_dict={}
	for model in model_dict:
		if not model in stl_dict: 
			imodel=stl_load()
			print("test",model,model_dict[model])
			imodel.load_stl(os.path.abspath('')+"/"+model_dict[model])
			stl_dict[model]=imodel
	return stl_dict
